...
def draw_move(board):
	...
	cnt = len(free)
	print('LEN(FREE) ', len(free))
	if cnt > 0:	# if the list is not empty, choose a place for 'X' and set it
		this = randrange(cnt) #aqui sorteia uma das tuplas livres
		row, col = free[this] #aqui faz a o recebimento de valor por ordem (cada tupla tem dois valores)
		board[row][col] = 'X'
